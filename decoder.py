...
class Decoder(BaseModel):
    ...
    def _build_loss(self, lr, logits, clip=(-1.25, 1.25), zero_weights_padding=False):
        weights = tf.cast(tf.not_equal(self.inputs_[:, :-1], 1), tf.dtypes.float32)
        if zero_weights_padding:
            weights = tf.cast(tf.not_equal(self.inputs_[:, :-1], 3), tf.dtypes.float32)        
    ...
    def _build_model(self, hparams, memory, enc_final_state, mem_seq):
        ...
        self.decoder_cells = self._generate_rnn_cell("GRU", hparams['num_units'], hparams['rnn_layers'], hparams['keep_prob'])
